      ...
      <div className="bg-[var(--surface)] rounded-[var(--r-xl)] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border border-[var(--line)]">
        ...
        >
          <div className="flex items-center gap-[14px]">
            <div className="w-[44px] h-[44px] rounded-[12px] bg-white/10 flex items-center justify-center text-[20px] font-bold border border-white/20 shadow-inner shrink-0 font-serif">
              {data?.hoTen ? data.hoTen.charAt(0).toUpperCase() : <User className="w-5 h-5 text-[var(--teal)]" />}
            </div>
            <div>
        ...
        {/* Content */}
        <div className="p-[24px] overflow-y-auto flex-1 space-y-[20px] bg-[var(--surface-bg)]">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center text-[var(--mute)]">
              <Loader2 className="w-8 h-8 animate-spin text-[var(--navy)] mb-2.5" />
              <p className="text-[13px] font-medium font-sans">Đang truy xuất chi tiết hồ sơ bệnh nhân...</p>
            </div>
          ) : error ? (
            <div className="p-4 bg-[var(--rose-soft)] text-[var(--rose)] rounded-[var(--r-lg)] text-center font-medium border border-[var(--rose)]/20 text-[13px]">
              {error}
            </div>
          ) : data ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
              {/* Section 1: Hành chính & Liên hệ */}
              <div className="bg-[var(--surface)] p-[18px] rounded-[var(--r-lg)] border border-[var(--line)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:border-[var(--line-strong)] transition-all duration-200 space-y-3">
                <h3 className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--navy)] flex items-center gap-2 border-b border-[var(--line-soft)] pb-2.5">
                  <User className="w-[14px] h-[14px] text-[var(--teal)]" /> Hành chính &amp; liên hệ
                </h3>
                <div className="space-y-2.5 text-[13px] font-sans">
                  <div className="flex justify-between py-1 border-b border-[var(--line-soft)]">
              ...
              <div className="bg-[var(--surface)] p-[18px] rounded-[var(--r-lg)] border border-[var(--line)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:border-[var(--line-strong)] transition-all duration-200 space-y-3">
                ...
                <div className="space-y-2.5 text-[13px] font-sans">
                  ...
                  <div className="py-1 border-b border-[var(--line-soft)]">
                    <span className="text-[var(--mute)] block mb-1.5">Chẩn đoán bác sĩ:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {parseDiag(data.chanDoan).map((d, i) => (
                        <span key={i} className="px-2 py-0.5 bg-[var(--teal-soft)] text-[var(--teal-deep)] rounded-[6px] text-[11.5px] font-semibold border border-[var(--teal)]/20">
                          {d}
                        </span>
                      ))}
                      {data.chanDoanKhac && (
                        <span className="px-2 py-0.5 bg-[var(--navy-50)] text-[var(--navy)] rounded-[6px] text-[11.5px] font-semibold border border-[var(--navy)]/20">
                          {data.chanDoanKhac}
                        </span>
                      )}
                ...
                <div className="space-y-2.5 text-[13px] font-sans">
                  ...
                  <div className="flex justify-between py-1 border-b border-[var(--line-soft)]">
                    ...
                    <span className="font-bold">
                      {data.nhom ? (
                        <span className={`px-[8px] py-[3px] rounded-[6px] text-[11px] font-bold uppercase tracking-wide border ${data.nhom === "A" ? "bg-[var(--rose-soft)] text-[var(--rose)] border-[var(--rose)]/20" : "bg-[var(--amber-soft)] text-[var(--amber)] border-[var(--amber)]/20"}`}>
                          Nhóm {data.nhom}
                        </span>
                      ) : "—"}
                    </span>
              ...
              <div className="bg-[var(--surface)] p-[18px] rounded-[var(--r-lg)] border border-[var(--line)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:border-[var(--line-strong)] transition-all duration-200 space-y-3">
                ...
                <div className="space-y-2.5 text-[13px] font-sans">
                  <div className="flex justify-between py-1 border-b border-[var(--line-soft)]">
                    ...
                    <span className="font-mono font-bold text-[#7c3aed] bg-[#f3eaf8] px-2 py-0.5 rounded-[6px] border border-[#e9d5ff]">
                  ...
                  <div className="flex justify-between py-1 border-b border-[var(--line-soft)]">
                    ...
                    <span className="font-medium text-[var(--ink)]">
                      {data.daDon ? <span className="text-[var(--teal-deep)] font-bold flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 inline text-[var(--teal)]" /> Đã đón viện</span> : "Chưa đến viện"}
                    </span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-[var(--line-soft)]">
              ...
              {/* Ghi chú nội bộ / HIS */}
              {data.ghiChuMat2 && (
                <div className="col-span-1 md:col-span-2 bg-[var(--amber-soft)] p-[16px] border border-[var(--amber)]/20 rounded-[var(--r-lg)] shadow-[var(--shadow-xs)]">
                  <h4 className="text-[11px] font-bold text-[var(--amber)] uppercase tracking-[0.14em] mb-2 flex items-center gap-1.5">
                    <FileText className="w-[14px] h-[14px] text-[var(--amber)]" /> Ghi chú nội bộ &amp; kết quả HIS
                  </h4>
                  <pre className="text-[12.5px] text-[var(--ink)] whitespace-pre-wrap font-sans bg-[var(--surface)] p-[14px] rounded-[10px] border border-[var(--line)] shadow-2xs leading-[1.5]">
                    {data.ghiChuMat2}
                  </pre>
                </div>
              )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-[var(--surface)] rounded-[var(--r-xl)] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border border-[var(--line)]">
        ...
        >
          <div className="flex items-center gap-[14px]">
            <div className="w-[44px] h-[44px] rounded-[12px] bg-white/10 flex items-center justify-center border border-white/20 shadow-inner shrink-0">
              <Clock className="w-[20px] h-[20px] text-[var(--teal)] animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2.5">
                <h2 className="font-serif text-[20px] font-bold tracking-[-0.02em] text-white">
        ...
        {/* Content */}
        <div className="p-[24px] overflow-y-auto flex-1 bg-[var(--surface-bg)]">
          {loading && !data ? (
            <div className="py-24 flex flex-col items-center justify-center text-[var(--mute)]">
              <Loader2 className="w-8 h-8 animate-spin text-[var(--navy)] mb-3" />
            ...
            <div className="relative pl-[28px] space-y-[16px] before:absolute before:left-[11px] before:top-3 before:bottom-3 before:w-[2px] before:bg-[var(--line-strong)]">
              {data.events.map((ev: any) => (
                <div key={ev.id} className="relative group animate-fade-in">
                  {/* Timeline node icon */}
                  <div className="absolute -left-[28px] top-[14px] w-[24px] h-[24px] rounded-full bg-[var(--surface)] border-2 border-[var(--line-strong)] shadow-[var(--shadow-xs)] flex items-center justify-center z-10 group-hover:border-[var(--teal)] group-hover:scale-110 transition-all">
                  ...
                  <div className="bg-[var(--surface)] rounded-[var(--r-lg)] border border-[var(--line)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:border-[var(--line-strong)] transition-all duration-200 overflow-hidden">
                    ...
                    <div className="bg-[var(--surface-soft)] px-[16px] py-[10px] border-b border-[var(--line-soft)] flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-[10px]">
                        {/* Avatar */}
                        <div
                          className={`w-[32px] h-[32px] rounded-[8px] flex items-center justify-center font-mono font-bold text-[12px] shadow-2xs border ${getAvatarStyle(
                            ev.userInfo?.role || "",
                            ev.userInfo?.isSystem
                          )}`}
                        >
                          {ev.userInfo?.isSystem ? <Zap className="w-4 h-4" /> : (ev.userInfo?.name || "?").charAt(0).toUpperCase()}
                        </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-[10.5px] font-semibold px-[7px] py-[2px] rounded-[6px] bg-[var(--line-soft)] text-[var(--ink-soft)] border border-[var(--line)]">
                      ...
                      {/* Thời gian */}
                      <div className="font-mono text-[11px] font-medium text-[var(--ink-soft)] bg-[var(--surface)] px-[10px] py-[4px] rounded-[6px] border border-[var(--line)] shadow-[var(--shadow-xs)] flex items-center gap-1.5">
                    ...
                    <div className="p-[16px] space-y-[12px]">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <h4 className="font-sans font-bold text-[13.5px] text-[var(--ink)] flex items-center gap-2">
                          {ev.action}
                        </h4>
                        {ev.badge && (
                          <span className={`font-mono text-[10px] font-bold uppercase tracking-[0.05em] px-[8px] py-[3px] rounded-[6px] border ${getBadgeStyle(ev.badge.cls, ev.type)}`}>
                            {ev.badge.label}
                          </span>
                        )}
                      </div>
                      ...
                      {ev.message && (
                        <div className="bg-[var(--surface-soft)] p-[12px] rounded-[10px] border border-[var(--line)] text-[12.5px] text-[var(--ink)] font-sans leading-[1.5] shadow-2xs">
                          <div className="flex items-start gap-2.5">
                            <MessageSquare className="w-[14px] h-[14px] text-[var(--teal)] mt-0.5 shrink-0" />
                      ...
                      {ev.changes && ev.changes.length > 0 && (
                        <div className="border border-[var(--line)] rounded-[10px] overflow-hidden bg-[var(--surface)]">
                          <div className="bg-[var(--surface-soft)] px-[14px] py-[8px] border-b border-[var(--line)] flex items-center justify-between">
                          ...
                          <div className="divide-y divide-[var(--line-soft)] max-h-[240px] overflow-y-auto">
                            {ev.changes.map((ch: any, idx: number) => (
                              <div key={idx} className="px-[14px] py-[9px] text-[12.5px] flex items-center justify-between hover:bg-[var(--surface-soft)] transition-colors">
                                ...
                                <span className="font-mono font-semibold text-[var(--navy)] w-3/5 text-right break-words pl-3">
                                  {ch.displayValue}
                                </span>
                              </div>
                            ))}
