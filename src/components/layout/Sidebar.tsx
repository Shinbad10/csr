...
export default function Sidebar({ open, setOpen }: { open?: boolean; setOpen?: (v: boolean) => void }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed lg:relative top-0 h-screen ${collapsed ? "w-[72px]" : "w-[248px]"} shrink-0 bg-white border-r border-[var(--line)] flex flex-col z-50 transition-all duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        {/* Brand */}
        <div className={`h-16 flex items-center gap-3 border-b border-[var(--line-soft)] shrink-0 transition-all duration-300 ${collapsed ? "px-0 justify-center" : "px-4 justify-between lg:justify-start"}`}>
          <div className={`flex items-center ${collapsed ? "justify-center w-full" : ""}`}>
            ...
            <div className={`leading-tight overflow-hidden transition-all duration-300 ${collapsed ? "hidden" : "max-w-[150px] opacity-100 ml-2.5"}`}>
      ...
      <div className={`border-b border-[var(--line-soft)] bg-[var(--surface-soft)] transition-all duration-300 ${collapsed ? "p-3 flex justify-center items-center" : "p-3"}`}>
        <div className={`overflow-hidden transition-all duration-300 ${collapsed ? "hidden" : "max-h-[20px] opacity-100 mb-1.5"}`}>
        ...
        {collapsed ? (
          <div className="relative group w-[38px] h-[38px] shrink-0 rounded-[var(--r-md)] bg-[var(--navy-50)] text-[var(--navy)] border border-[var(--navy-100)] flex items-center justify-center transition-all duration-300 cursor-pointer">
            ...
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-3 py-2 bg-[var(--navy-deep)] text-white text-[13px] font-bold rounded-lg shadow-xl whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-200 z-[999] flex items-center gap-1.5 border border-[var(--navy-100)]">
              <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 bg-[var(--navy-deep)] rotate-45 border-l border-b border-[var(--navy-100)]" />
      ...
      {/* Nav */}
      <nav data-tour="db-nav" className={`flex-1 space-y-5 transition-all duration-300 ${collapsed ? "px-2 py-4 flex flex-col items-center" : "px-3 py-4"}`}>
        {visibleGroups.map((group) => (
          <div key={group.title} className="space-y-1 w-full flex flex-col">
            <div className={`font-sans font-extrabold text-[9px] uppercase tracking-[0.15em] text-[var(--mute)] whitespace-nowrap overflow-hidden transition-all duration-300 ${collapsed ? "hidden" : "max-w-[200px] opacity-100 h-auto px-2.5"}`}>
              {group.title}
            </div>
                ...
                return (
                  <div key={item.href} className="relative group flex justify-center w-full">
                    <Link href={item.href} onClick={() => setOpen && setOpen(false)} title={collapsed ? undefined : item.label}
                      ...
                      <span className={`text-[13px] whitespace-nowrap overflow-hidden transition-all duration-300 ${collapsed ? "hidden" : "max-w-[150px] opacity-100"}`}>
                        {item.label}
                      </span>
                    </Link>
                    {collapsed && (
