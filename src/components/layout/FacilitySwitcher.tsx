...
export default function FacilitySwitcher({ className = "", variant = "dark" }: FacilitySwitcherProps) {
    ...
    return (
      <div className={`relative ${className}`} ref={ref} suppressHydrationWarning>
        ...
        {isDark ? (
          ...
          <button
            ...
            className={`w-full p-2.5 rounded-xl border transition-all cursor-pointer select-none flex items-center justify-between gap-2.5 ${
              open
                ? "bg-white border-[var(--teal)] shadow-sm ring-2 ring-[var(--teal)]/15"
                : "bg-white border-[var(--line-strong)] hover:border-[var(--teal)] hover:bg-[var(--surface-soft)] shadow-2xs"
            }`}
            title={facilityName}
          >
            <div className="flex items-center gap-2.5 min-w-0 flex-1 text-left" suppressHydrationWarning>
              <div className="w-7 h-7 rounded-lg bg-[var(--navy-50)] text-[var(--navy)] flex items-center justify-center shrink-0 border border-[var(--navy-100)]">
                <Building2 className="w-4 h-4" />
              </div>
              <div className="min-w-0" suppressHydrationWarning>
                <div className="text-[12.5px] font-bold text-[var(--ink)] truncate leading-tight" suppressHydrationWarning>{facilityName}</div>
                <div className="text-[9.5px] font-mono font-bold text-[var(--teal-deep)] uppercase tracking-wider mt-0.5">
                  Đang hoạt động
                </div>
              </div>
            </div>
            <div className="w-6 h-6 rounded-full bg-[var(--surface-soft)] border border-[var(--line)] flex items-center justify-center shrink-0">
              <ChevronDown className={`w-3.5 h-3.5 text-[var(--mute)] transition-transform duration-200 ${open ? "rotate-180 text-[var(--teal-deep)]" : ""}`} />
            </div>
          </button>
        )}

        {/* Drawer Mobile: Inline Accordion Expansion (Doesn't cover menu) */}
        {!isDark && open && (
          <div
            suppressHydrationWarning
            className="mt-2 bg-white border border-[var(--line-strong)] rounded-2xl shadow-xs p-1.5 text-[var(--ink)] overflow-hidden animate-fade-in"
          >
            <div className="px-2 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-[var(--mute)] font-mono border-b border-[var(--line-soft)] mb-1 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between gap-2 px-2.5 py-2 rounded-xl text-[12px] font-bold transition-all text-left cursor-pointer ${
                      active
                        ...
                        : "text-[var(--ink)] hover:bg-[var(--navy-50)] hover:text-[var(--navy)]"
                    }`}
                  >
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      <div
                        className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 ${
                          active ? "bg-white/20 text-[var(--teal)]" : "bg-[var(--surface-soft)] text-[var(--mute)]"
                        }`}
                      >
                        <Building2 className="w-3 h-3" />
                      </div>
        ...
        {isDark && open && (
          <div
            suppressHydrationWarning
            className="absolute left-0 top-full mt-2 w-[290px] z-50 bg-white border border-[var(--line-strong)] rounded-2xl shadow-2xl p-2 animate-dropdown text-[var(--ink)]"
          >
            <div className="px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-[var(--mute)] font-mono border-b border-[var(--line-soft)] mb-1.5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div
                        className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${
                          active ? "bg-white/20 text-[var(--teal)]" : "bg-[var(--surface-soft)] text-[var(--mute)] border border-[var(--line)]"
                        }`}
                      >
  ...
  return isDark ? (
    ...
    <div
      suppressHydrationWarning
      className={`p-2.5 rounded-xl bg-white border border-[var(--line-strong)] shadow-2xs flex items-center gap-2.5 ${className}`}
      title={facilityName}
    >
      <div className="w-7 h-7 rounded-lg bg-[var(--navy-50)] text-[var(--navy)] flex items-center justify-center shrink-0 border border-[var(--navy-100)]">
        <Building2 className="w-4 h-4" />
